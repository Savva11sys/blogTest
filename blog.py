...
def delete_post (username, content):
    """Удаление поста."""
    with sqlite3.connect(DATABASE) as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT id FROM users WHERE name = ?', (username,))
...
def dellit_post_gui():
    """Окно для удаления поста."""
    username = simpledialog.askstring("Удалить пост","Введите имя пользоывтеля:")
    if username:
        content = simpledialog.askstring("Удалить.")
        if content:
            delete_post(username,content)
...
delete_blog_button = tk.Button (root, text="Удалить пост", command=root.quit, width=25)
delete_blog_button.pack(pady=6)
